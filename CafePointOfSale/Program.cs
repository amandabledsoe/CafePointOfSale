...
using CafePointOfSale;

Dictionary<Product, int> ourInventory = new Dictionary<Product, int>();
Inventory inventory = new Inventory(ourInventory);
//BAKERY ITEMS
ourInventory.Add(new Product(inventory, "Talon's Loaf of Lemon", Category.BakeryItem, "Delicious & Royal Lemony Goodness", 1.99m), 50);
ourInventory.Add(new Product(inventory, "Agent JSmith Plain Bagel", Category.BakeryItem, "The Choice of John Smith's & Jane Doe's everywhere", 1.50m), 72);
ourInventory.Add(new Product(inventory, "DLux Double Chocolate Brownie", Category.BakeryItem, "The Coder's Delight", 2.99m), 420);
ourInventory.Add(new Product(inventory, "The Kaige Kupkake", Category.BakeryItem, "Pairs Well with Any & All Coffees", 2.99m), 120);
ourInventory.Add(new Product(inventory, "Christy's Boysenberry Scone", Category.BakeryItem, "Unique & Flavorful", 2.99m), 50);
ourInventory.Add(new Product(inventory, "Hussein's Niche Quiche", Category.BakeryItem, "Tastes great & fits with any meal", 2.99m), 36);
//COFFEE ITEMS
ourInventory.Add(new Product(inventory, "GWiz Espresso Shot", Category.CoffeeItem, "Pure Craziness in a Cup", 4.53m), 500);
ourInventory.Add(new Product(inventory, "Bret Americano", Category.CoffeeItem, "Coffeee + Espresso", 2.19m), 250);
ourInventory.Add(new Product(inventory, "Dominion's Decaf", Category.CoffeeItem, "The Coffee for Those Who Dont Like Coffee", 2.19m), 20);
ourInventory.Add(new Product(inventory, "Alex's Mustachio Matcha Latte", Category.CoffeeItem, "Smooth but Different", 2.19m), 200);
ourInventory.Add(new Product(inventory, "Karen's Kombucha", Category.CoffeeItem, "Peppy, Sweet, & Helps Ya Get Sh!t Done!", 2.19m), 200);
ourInventory.Add(new Product(inventory, "Antonio's Special Drank", Category.CoffeeItem, "Mystery Deliciosity with Caffeine", 2.19m), 50);

UserCart thisUserCart = new UserCart();
                                ...
                                switch (payNum)
                                {
                                    case 1:
                                        ...
                                        while (usingCreditCard)
                                        {
                                            ...
                                            CreditCardPayment thisCreditCardPayment = new CreditCardPayment();
                                        }
                                        break;
                                    case 2:
                                        //Check will go here
...
static void CashTransaction(decimal orderTotal, Dictionary<Product, int> userCart)
{
    ...
    while (gettingCashPayment)
    {
        ...
        while (gettingChange)
        {
            ...
            {
                Console.WriteLine("Sorry, looks like that amount won't cover the total. Let's enter a new amount.");
                gettingChange = false;
            }
        }
    }
